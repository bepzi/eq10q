...
#define K_BIAS_GAIN \
    0.0891250938133745507219174442070652730762958526611328125f  // A trick to change the dinamic
                                                                // range of the fast_db2lin10 method
                                                                // for gate plugin
typedef struct {
    // Plugin ports
    float *key_listen;
...
static LV2_Handle instantiateDyn(const LV2_Descriptor *descriptor, double s_rate, const char *path,
...
#define DENORMAL_TO_ZERO_FLOAT(x) \
    if (fabs(x) < (1e-30)) x = 0.f;  // Min float is 1.1754943e-38
...
// Processor vars (only for gate)
    ...
    const float Kac =
        pow((range_lin / (1.0f - range_lin)) * ((1.0f - 0.9f) / 0.9f),
            1.0f / (attack * 0.001f * sample_rate));  // Attack constant for S curve in gate
#endif
...
// Processor vars (only COMPRESSOR)
    ...
    float g_error = 0.0f;
    float scl = 0.0f;
    const float range_lin = pow(10, -12 * 0.05);  // By various tests -12 dB is the optimal setting
                                                  // for the release time of a opto-compressor
    const float Kdc =
        pow((range_lin / (1.0f - range_lin)) * ((1.0f - 0.6f) / 0.6f),
            1.0f / (decay * 0.001f * sample_rate));  // Decay constant for S curve in compressor
#endif
    ...
    for (uint32_t i = 0; i < sample_count; ++i) {
...
#if defined(PLUGIN_IS_COMPRESSOR) || defined(PLUGIN_IS_COMPRESSOR_WITH_SC)
        dToFiltersChain =
            ((double)input_preL * (1.0 - FeedBack) + plugin_data->PGAOut_L * FeedBack) *
                (1.0 - SideChainActive) +
            (double)input_sc * SideChainActive;
#else
...
#if defined(PLUGIN_IS_COMPRESSOR) || defined(PLUGIN_IS_COMPRESSOR_WITH_SC)
        dToFiltersChain = (((double)(input_preL + input_preR) * (1.0 - FeedBack) +
                            (plugin_data->PGAOut_L + plugin_data->PGAOut_R) * FeedBack) *
                           0.5) *
                              (1.0 - SideChainActive) +
                          (double)input_sc * SideChainActive;
#else
        ...
        // Detector signal used for Threshold VU-meter in both gate and compressor, also used in
        // expander hold
...
#else
        x_dB = 20.0f *
               log10(fabs(input_filtered) + 0.00001f);  // Add -100dB constant to avoid zero crozing
#endif
...
//===================== GATE CODE ================================
        ...
        if (knee_range < -knee) {
            ...
            // On Knee
            y_dB = x_dB +
                   ((1.0 - ratio) * (x_dB - threshold - knee / 2) * (x_dB - threshold - knee / 2)) /
                       (2 * knee);
        }
        if (y_dB < x_dB + range) {
            y_dB = x_dB + range;
...
// Linear gain computing
        ...
        gain_reduction =
            K_BIAS_GAIN *
            Fast_dB2Lin10(y_dB - x_dB +
                          22);  // 22dB bias compensated with K_BIAS_GAIN linear multiplication.
                                // This allows a good response of Fast_dB2Lin10 at -90 dB range
#else
...
//=================== COMPRESSOR CODE ============================
        ...
        if (knee_range < -knee) {
            ...
            // On Knee
            y_dB = x_dB + ((1.0f / ratio - 1.0f) * (x_dB - threshold + knee / 2.0f) *
                           (x_dB - threshold + knee / 2.0f)) /
                              (2.0f * knee);
        }
...
// Linear gain computing
        ...
        if (gain_reduction > g) {
            ...
            if (bIsOptoCompressor) {
                gain_reduction =
                    1.0f / (1.0f + Kdc * ((1.0f - g) / (g + 1e-8f)));  // S-Curve release
            } else {
            ...
            scl = punch * (g_error * plugin_data->g_error0) /
                  ((1.0f - gain_reduction + 1e-8f) * (1.0f - gain_reduction + 1e-8f));
        ...
        plugin_data->output[0][i] =
            input_filtered * (KeyListen) +
            (input_preL * (1.0f - DryWet) + makeup * (float)plugin_data->PGAOut_L * DryWet) *
                (1 - KeyListen);
        ...
        plugin_data->output[1][i] =
            input_filtered * (KeyListen) +
            (input_preR * (1.0f - DryWet) + makeup * (float)plugin_data->PGAOut_R * DryWet) *
                (1 - KeyListen);
#endif
    }
    plugin_data->g = g;
    ...
    *(plugin_data->gainreduction) =
        1.0 / gr_meter +
        plugin_data->noise;  // + ((float)(rand() % 100)/100.0);; //OK esta en lineal
    *(plugin_data->fVuIn) = ComputeVu(plugin_data->InputVu[0], sample_count);
}

static const LV2_Descriptor dynDescriptor = {DYN_URI, instantiateDyn, connectPortDyn, NULL,
                                             runDyn,  NULL,           cleanupDyn,     NULL};
