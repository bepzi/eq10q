...
KnobWidget2::KnobWidget2(float fMin, float fMax, std::string sLabel, std::string sUnits,
                         const char* knobIconPath, int iType, bool snap2ZerodB)
    : m_fMin(fMin),
      m_fMax(fMax),
    ...
    set_size_request((int)(1.5 * (double)(m_image_ptr->get_width())),
                     TEXT_SIZE + (int)(1.5 * (double)(m_image_ptr->get_height())));

    add_events(Gdk::BUTTON_PRESS_MASK | Gdk::BUTTON_RELEASE_MASK | Gdk::POINTER_MOTION_MASK |
               Gdk::SCROLL_MASK | Gdk::LEAVE_NOTIFY_MASK);
    signal_button_press_event().connect(sigc::mem_fun(*this, &KnobWidget2::on_button_press_event),
                                        true);
    signal_button_release_event().connect(
        sigc::mem_fun(*this, &KnobWidget2::on_button_release_event), true);
    signal_scroll_event().connect(sigc::mem_fun(*this, &KnobWidget2::on_scrollwheel_event), true);
    signal_motion_notify_event().connect(sigc::mem_fun(*this, &KnobWidget2::on_mouse_motion_event),
                                         true);
    signal_leave_notify_event().connect(sigc::mem_fun(*this, &KnobWidget2::on_mouse_leave_widget),
                                        true);
}

KnobWidget2::~KnobWidget2() {}

KnobWidget2::signal_KnobChanged KnobWidget2::signal_changed() { return m_KnobChangedSignal; }
...
double KnobWidget2::get_value() { return m_Value; }
...
bool KnobWidget2::on_mouse_motion_event(GdkEventMotion* event) {
    if (bMotionIsConnected) {
        ...
        switch (m_TypeKnob) {
            case KNOB_TYPE_FREQ:
                increment =
                    m_slowMultiplier * MOUSE_EVENT_PERCENT * (m_fMax - m_fMin) * 0.0002 * m_Value;
                break;
...
bool KnobWidget2::on_expose_event(GdkEventExpose* event) {
    ...
    if (window) {
        ...
        if (m_focus) {
            Cairo::RefPtr<Cairo::RadialGradient> glow_gradient_ptr =
                Cairo::RadialGradient::create(KNOB_CENTER_X, KNOB_CENTER_Y, KNOB_RADIUS - 0.1,
                                              KNOB_CENTER_X, KNOB_CENTER_Y, KNOB_RADIUS + 0.1);

            glow_gradient_ptr->add_color_stop_rgba(0, 0.4, 0.6, 0.8, 0.6);
            glow_gradient_ptr->add_color_stop_rgba(1, BACKGROUND_R, BACKGROUND_G, BACKGROUND_B,
                                                   0.1);
        ...
        Cairo::RefPtr<Cairo::RadialGradient> bkg_gradient_ptr =
            Cairo::RadialGradient::create(KNOB_CENTER_X, KNOB_CENTER_Y, KNOB_RADIUS - 0.08,
                                          KNOB_CENTER_X, KNOB_CENTER_Y, KNOB_RADIUS + 0.1);
        ...
        // Draw colored circle
        Cairo::RefPtr<Cairo::RadialGradient> rad_gradient_ptr =
            Cairo::RadialGradient::create(KNOB_CENTER_X, KNOB_CENTER_Y, KNOB_RADIUS - 0.08,
                                          KNOB_CENTER_X, KNOB_CENTER_Y, KNOB_RADIUS + 0.1);
        ...
        // Draw the knob icon
        cr->set_source(m_image_surface_ptr, -m_image_surface_ptr->get_width() / 2,
                       -m_image_surface_ptr->get_height() / 2);
        cr->rectangle(-m_image_surface_ptr->get_width() / 2, -m_image_surface_ptr->get_height() / 2,
                      m_image_surface_ptr->get_width(), m_image_surface_ptr->get_height());
        cr->clip();
        cr->paint();
        cr->restore();
    }
