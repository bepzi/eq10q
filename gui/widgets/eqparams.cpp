...
//#define USE_SLV2 ///TODO: check this-> esta donant problemes amb algunes versions de ardour, ull
//slv2 esta anticuat!!!
#ifdef USE_SLV2
...
EqParams::EqParams(int iNumBands) : m_iNumberOfBands(iNumBands) {
    m_ptr_BandArray = (EqBandStruct*)malloc(sizeof(EqBandStruct) * m_iNumberOfBands);
}

EqParams::~EqParams() { free(m_ptr_BandArray); }

bool EqParams::getBandEnabled(int iBand) { return m_ptr_BandArray[iBand].bIsEnabled; }

float EqParams::getBandFreq(int iBand) { return m_ptr_BandArray[iBand].fFreq; }

float EqParams::getBandGain(int iBand) { return m_ptr_BandArray[iBand].fGain; }

float EqParams::getBandQ(int iBand) { return m_ptr_BandArray[iBand].fQ; }

int EqParams::getBandType(int iBand) { return m_ptr_BandArray[iBand].iType; }

float EqParams::getInputGain() { return m_fInGain; }

float EqParams::getOutputGain() { return m_fOutGain; }

void EqParams::setBandEnabled(int iBand, bool bIsEnabled) {
    m_ptr_BandArray[iBand].bIsEnabled = bIsEnabled;
}

void EqParams::setBandFreq(int iBand, float fFreq) { m_ptr_BandArray[iBand].fFreq = fFreq; }

void EqParams::setBandGain(int iBand, float fGain) { m_ptr_BandArray[iBand].fGain = fGain; }

void EqParams::setBandQ(int iBand, float fQ) { m_ptr_BandArray[iBand].fQ = fQ; }

void EqParams::setBandType(int iBand, int iType) { m_ptr_BandArray[iBand].iType = iType; }

void EqParams::setInputGain(float fInGain) { m_fInGain = fInGain; }

void EqParams::setOutputGain(float fOutGain) { m_fOutGain = fOutGain; }
...
void EqParams::loadFromTtlFile(const char* uri) {
    ...
    SLV2Plugin lv2ThisPlugin =
        slv2_plugins_get_by_uri(lv2Plugins, slv2_value_new_uri(lv2World, uri));
