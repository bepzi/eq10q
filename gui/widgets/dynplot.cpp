...
double PlotDynCurve::dB2PixelsX(double db) {
    const double m = ((double)width - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET) /
                     ((double)(DATA_RANGE_MAX - DATA_RANGE_MIN));
    const double n = CURVE_MARGIN + CURVE_TEXT_OFFSET - m * DATA_RANGE_MIN;
    return m * db + n;
}

double PlotDynCurve::dB2PixelsY(double db) {
    const double m = (2 * CURVE_MARGIN + CURVE_TEXT_OFFSET - (double)height) /
                     ((double)(DATA_RANGE_MAX - DATA_RANGE_MIN));
...
bool PlotDynCurve::on_expose_event(GdkEventExpose* event) {
    ...
    if (window) {
        ...
        cr->save();
        cr->rectangle(CURVE_MARGIN + CURVE_TEXT_OFFSET + 0.5, CURVE_MARGIN + 0.5,
                      width - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET,
                      height - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET);
        cr->clip();
        cr->begin_new_sub_path();
        ...
        cr->line_to(dB2PixelsX(m_InputVu + prange),
                    dB2PixelsY(m_InputVu + prange - m_GainReduction + m_Makeup));
        cr->line_to(dB2PixelsX(m_InputVu + prange), dB2PixelsY(DATA_RANGE_MIN));
        cr->close_path();
        bkg_gradient_ptr = Cairo::LinearGradient::create(
            dB2PixelsX(DATA_RANGE_MIN), dB2PixelsY(DATA_RANGE_MIN), dB2PixelsX(m_InputVu + prange),
            dB2PixelsY(DATA_RANGE_MIN));
        ...
        if (m_bIsCompressor) {
            for (double x_dB = DATA_RANGE_MIN; x_dB <= DATA_RANGE_MAX; x_dB += 1.0) {
                ...
                if (knee_range < -m_Knee) {
                    ...
                    // On Knee
                    y_dB = x_dB + ((1.0 / m_Ratio - 1.0) * (x_dB - m_Threshold + m_Knee / 2.0) *
                                   (x_dB - m_Threshold + m_Knee / 2.0)) /
                                      (2.0 * m_Knee);
                }
                y_dB += m_Makeup;
            ...
            for (double x_dB = DATA_RANGE_MIN; x_dB <= DATA_RANGE_MAX; x_dB += 1.0) {
                ...
                if (knee_range < -m_Knee) {
                    ...
                    // On Knee
                    y_dB = x_dB + ((1.0 - m_Ratio) * (x_dB - m_Threshold - m_Knee / 2) *
                                   (x_dB - m_Threshold - m_Knee / 2)) /
                                      (2 * m_Knee);
                }
        ...
        cr->line_to(CURVE_MARGIN + CURVE_TEXT_OFFSET + 0.5,
                    height - CURVE_MARGIN - CURVE_TEXT_OFFSET + 0.5);
