...
public:
    ...
    void gui_port_event(LV2UI_Handle ui, uint32_t port, uint32_t buffer_size, uint32_t format,
        ...
        // Atom event from DSP
        if ((int)port ==
            (PORT_OFFSET + 2 * m_iNumOfChannels + 5 * m_iNumOfBands + 2 * m_iNumOfChannels)) {
            ...
            if (format == uris.atom_eventTransfer && atom->type == uris.atom_Object) {
                ...
                if (obj->body.otype == uris.atom_sample_rate_response) {
                    ...
                    const int n_props =
                        lv2_atom_object_get(obj, uris.atom_sample_rate_key, &samplerate_val, NULL);

                    if (n_props != 1 || samplerate_val->type != uris.atom_Double) {
                        std::cout << "Atom Object does not have the required properties "
                                     "(sample-rate) with correct types"
                                  << std::endl;
                    } else {
                    ...
                    const int n_props =
                        lv2_atom_object_get(obj, uris.atom_fft_data_key, &fftdata_val, NULL);

                    if (n_props != 1 || fftdata_val->type != uris.atom_Vector) {
                        std::cout << "Atom Object does not have the required properties (fft-data) "
                                     "with correct types"
                                  << std::endl;
                    } else {
                        ...
                        if (vec->body.child_type != uris.atom_Double) {
                            ...
                            const size_t n_elem =
                                ((fftdata_val->size - sizeof(LV2_Atom_Vector_Body)) /
                                 sizeof(double));
...
/// m_OutGain->setGain(data);
            ...
            default:
                ...
                if ((int)port >= (PORT_OFFSET + 2 * m_iNumOfChannels) &&
                    (int)port < (PORT_OFFSET + 2 * m_iNumOfChannels + m_iNumOfBands)) {
                ...
                else if ((int)port >= (PORT_OFFSET + 2 * m_iNumOfChannels + m_iNumOfBands) &&
                         (int)port < (PORT_OFFSET + 2 * m_iNumOfChannels + 2 * m_iNumOfBands)) {
                    /// m_BandCtlArray[(int)port - PORT_OFFSET - 2*m_iNumOfChannels -
                    /// m_iNumOfBands]->setFreq(data); m_Bode->setBandFreq((int)port - PORT_OFFSET -
                    /// 2*m_iNumOfChannels - m_iNumOfBands, data);
                    m_CurParams->setBandFreq(
                        (int)port - PORT_OFFSET - 2 * m_iNumOfChannels - m_iNumOfBands, data);
                    m_port_event_Curve = true;  // Marck port even boolean
                    m_port_event_Curve_Freq[(int)port - PORT_OFFSET - 2 * m_iNumOfChannels -
                                            m_iNumOfBands] = true;
#ifdef PRINT_DEBUG_INFO
                ...
                else if ((int)port >= (PORT_OFFSET + 2 * m_iNumOfChannels + 2 * m_iNumOfBands) &&
                         (int)port < (PORT_OFFSET + 2 * m_iNumOfChannels + 3 * m_iNumOfBands)) {
                    /// m_BandCtlArray[(int)port - PORT_OFFSET - 2*m_iNumOfChannels -
                    /// 2*m_iNumOfBands]->setQ(data); m_Bode->setBandQ((int)port - PORT_OFFSET -
                    /// 2*m_iNumOfChannels - 2*m_iNumOfBands, data);
                    m_CurParams->setBandQ(
                        (int)port - PORT_OFFSET - 2 * m_iNumOfChannels - 2 * m_iNumOfBands, data);
                    m_port_event_Curve = true;  // Marck port even boolean
                    m_port_event_Curve_Q[(int)port - PORT_OFFSET - 2 * m_iNumOfChannels -
                                         2 * m_iNumOfBands] = true;
#ifdef PRINT_DEBUG_INFO
                ...
                else if ((int)port >= (PORT_OFFSET + 2 * m_iNumOfChannels + 3 * m_iNumOfBands) &&
                         (int)port < (PORT_OFFSET + 2 * m_iNumOfChannels + 4 * m_iNumOfBands)) {
                    /// m_BandCtlArray[(int)port - PORT_OFFSET - 2*m_iNumOfChannels -
                    /// 3*m_iNumOfBands]->setFilterType(data); m_Bode->setBandType((int)port -
                    /// PORT_OFFSET - 2*m_iNumOfChannels - 3*m_iNumOfBands, data);
                    m_CurParams->setBandType(
                        (int)port - PORT_OFFSET - 2 * m_iNumOfChannels - 3 * m_iNumOfBands,
                        ((int)data) & 0xFF);
                    m_port_event_Curve = true;  // Marck port even boolean
                    m_port_event_Curve_Type[(int)port - PORT_OFFSET - 2 * m_iNumOfChannels -
                                            3 * m_iNumOfBands] = true;
#ifdef PRINT_DEBUG_INFO
                ...
                else if ((int)port >= (PORT_OFFSET + 2 * m_iNumOfChannels + 4 * m_iNumOfBands) &&
                         (int)port < (PORT_OFFSET + 2 * m_iNumOfChannels + 5 * m_iNumOfBands)) {
                    /// m_BandCtlArray[(int)port - PORT_OFFSET - 2*m_iNumOfChannels -
                    /// 4*m_iNumOfBands]->setEnabled(data > 0.5); m_Bode->setBandEnable((int)port -
                    /// PORT_OFFSET - 2*m_iNumOfChannels - 4*m_iNumOfBands, data > 0.5);
                ...
                else if ((int)port >= (PORT_OFFSET + 2 * m_iNumOfChannels + 5 * m_iNumOfBands) &&
                         (int)port < (PORT_OFFSET + 2 * m_iNumOfChannels + 5 * m_iNumOfBands +
                                      m_iNumOfChannels)) {
                    m_VuMeterIn->setValue(
                        (int)port - PORT_OFFSET - 2 * m_iNumOfChannels - 5 * m_iNumOfBands, data);
#ifdef PRINT_DEBUG_INFO
                ...
                // Connect VuOutput ports
                    ...
                    m_VuMeterOut->setValue((int)port - PORT_OFFSET - 2 * m_iNumOfChannels -
                                               5 * m_iNumOfBands - m_iNumOfChannels,
                                           data);
#ifdef PRINT_DEBUG_INFO
...
protected:
    ...
    Gtk::VBox m_CurveBypassBandsBox, m_MainBox, m_InGainBox, m_OutGainBox, m_FftCtlVBox,
        m_dBScaleBox, m_FftdBBox, m_StereoBox;
    ToggleButton m_BypassButton, m_FftRtaActive, m_FftSpecActive, m_dB10Scale, m_dB25Scale,
        m_dB50Scale, m_LRStereoMode, m_MSStereoMode;
    AbButton m_AButton;
    Gtk::Alignment m_FlatAlign, m_ABAlign, m_ButtonAAlign, m_BypassAlign, m_LoadAlign, m_SaveAlign,
        m_FftAlign, m_FftAlignInner, m_FftAlngGain, m_FftAlngRange, m_dBScaleAlign,
        m_dBScaleAlignInner, m_StereoInnerAlng, m_StereAlng;
...
private:
    ...
    bool m_bMutex, m_port_event_InGain, m_port_event_OutGain, m_port_event_Bypass,
        m_port_event_Curve;
    bool *m_port_event_Curve_Gain, *m_port_event_Curve_Freq, *m_port_event_Curve_Q,
        *m_port_event_Curve_Type, *m_port_event_Curve_Enable;
