...
TemplateWidget::TemplateWidget(main_window *m_ptr,
                               void (*f_set_ptr)(main_window *myptr, int b_ix, int type, float g,
                                                 float f, float Q),
                               void (*f_get_ptr)(main_window *myptr, int band_id,
                                                 BandParams &m_params))
    : b_load("Load Preset"),
      b_save("Save Preset"),
      b_remove("Delete Preset"),
      l_presets("Presets") {
    // External functions
...
void TemplateWidget::on_load_clicked() {
    ...
    if (current_preset != -1) {  // diferent de -1 implica k no estem introduint text
        ...
        if (!ofs)
            std::cerr << "Error: file can't be open";
        else {
            ofs.clear();
            ...
            for (int i = 0; i < 10; i++) {  // 10 es el nombre de bandes
                external_set_ptr(main_win_ptr, i, f_filter.fparams[i].type,
                                 f_filter.fparams[i].gain, f_filter.fparams[i].freq,
...
/*Si clickem el button save hem de fer el seguent:
  - Si no s'ha introduit un nom valid (entenent valid com no NUL i no repetit) donar un missatge
  d'error
    ...
    if (entry) {
        ...
        if (!presetname.empty()) {
            ...
            if (!ofs)
                ...
                f_filter.name_length =
                    (NAME_LONG - 1 > presetname.length() ? presetname.length() : NAME_LONG - 1);
...
void TemplateWidget::on_delete_clicked() {
    ...
    Gtk::MessageDialog dialog((Gtk::Window &)(*this->get_toplevel()),
                              "This will delete the selected preset, are you sure?",
 ...
 */
void TemplateWidget::on_combo_move() { current_preset = preset_combo.get_active_row_number(); }
