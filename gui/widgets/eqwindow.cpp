...
EqMainWindow::EqMainWindow(int iAudioChannels, int iNumBands, const char *uri,
    ...
    m_GainFaderIn =
        Gtk::manage(new KnobWidget2(-20.0, 20.0, "In Gain", "dB",
                                    (m_bundlePath + KNOB_ICON_FILE).c_str(), KNOB_TYPE_LIN, true));
    m_GainFaderOut =
        Gtk::manage(new KnobWidget2(-20.0, 20.0, "Out Gain", "dB",
                                    (m_bundlePath + KNOB_ICON_FILE).c_str(), KNOB_TYPE_LIN, true));
    ...
    m_FftRange = Gtk::manage(new KnobWidget2(
        20.0, 100.0, "Range", "dB", (m_bundlePath + KNOB_ICON_FILE_MINI).c_str(), KNOB_TYPE_LIN));
    m_FftGain = Gtk::manage(new KnobWidget2(-20.0, 20.0, "Gain", "dB",
                                            (m_bundlePath + KNOB_ICON_FILE_MINI).c_str(),
                                            KNOB_TYPE_LIN, true));
    ...
    for (int i = 0; i < m_iNumOfBands; i++) {
        m_BandCtlArray[i] =
            Gtk::manage(new BandCtl(i, &m_bMutex, m_bundlePath.c_str(), m_iNumOfChannels == 2));
        m_BandBox.pack_start(*m_BandCtlArray[i], Gtk::PACK_SHRINK);
        m_BandCtlArray[i]->signal_changed().connect(
            sigc::mem_fun(*this, &EqMainWindow::onBandChange));
        m_BandCtlArray[i]->signal_band_selected().connect(
            sigc::mem_fun(*this, &EqMainWindow::onBandCtlSelectBand));
        m_BandCtlArray[i]->signal_band_unselected().connect(
            sigc::mem_fun(*this, &EqMainWindow::onBandCtlUnselectBand));
        m_BandCtlArray[i]->signal_mid_side_changed().connect(
            sigc::mem_fun(*this, &EqMainWindow::onBandCtlMidSideChanged));
    }
    ...
    m_GainFaderOut->signal_changed().connect(
        sigc::mem_fun(*this, &EqMainWindow::onOutputGainChange));
    ...
    if (m_iNumOfChannels == 2) {
        m_LRStereoMode.signal_clicked().connect(
            sigc::mem_fun(*this, &EqMainWindow::onLeftRightModeSelected));
        m_MSStereoMode.signal_clicked().connect(
            sigc::mem_fun(*this, &EqMainWindow::onMidSideModeSelected));
    }

    // Load the EQ Parameters objects, the params for A curve will be loaded by host acording
    // previous session plugin state
...
void EqMainWindow::request_sample_rate() {
    // Request Sample rate
    int AtomPortNumber =
        PORT_OFFSET + 2 * m_iNumOfChannels + 5 * m_iNumOfBands + 2 * m_iNumOfChannels + 1;
    uint8_t obj_buf[64];
    lv2_atom_forge_set_buffer(&forge, obj_buf, sizeof(obj_buf));
    LV2_Atom_Forge_Frame frame;
    LV2_Atom *msg =
        (LV2_Atom *)lv2_atom_forge_object(&forge, &frame, 0, uris.atom_sample_rate_request);
    lv2_atom_forge_pop(&forge, &frame);
    write_function(controller, AtomPortNumber, lv2_atom_total_size(msg), uris.atom_eventTransfer,
                   msg);
}
...
void EqMainWindow::changeAB(EqParams *toBeCurrent) {
    ...
    for (int i = 0; i < m_iNumOfBands; i++) {
        ...
        // Because if there are some value that does not change between A/B curves this could be not
        // correctly updated
        m_BandCtlArray[i]->setFreq(m_CurParams->getBandFreq(i));
        m_BandCtlArray[i]->setGain(m_CurParams->getBandGain(i));
        m_BandCtlArray[i]->setEnabled(m_CurParams->getBandEnabled(i));
        ...
        write_function(controller, i + PORT_OFFSET + 2 * m_iNumOfChannels, sizeof(float), 0,
                       &aux);  // Gain
        aux = m_CurParams->getBandFreq(i);
        write_function(controller, i + PORT_OFFSET + 2 * m_iNumOfChannels + m_iNumOfBands,
                       sizeof(float), 0, &aux);  // Freq
        aux = m_CurParams->getBandQ(i);
        write_function(controller, i + PORT_OFFSET + 2 * m_iNumOfChannels + 2 * m_iNumOfBands,
                       sizeof(float), 0, &aux);  // Q
        aux = m_CurParams->getBandEnabled(i);
        write_function(controller, i + PORT_OFFSET + 2 * m_iNumOfChannels + 4 * m_iNumOfBands,
                       sizeof(float), 0, &aux);  // Enable
        aux = m_CurParams->getBandType(i);
        write_function(controller, i + PORT_OFFSET + 2 * m_iNumOfChannels + 3 * m_iNumOfBands,
                       sizeof(float), 0, &aux);  // Filter type
    }
}
...
void EqMainWindow::onButtonFlat() {
    ...
    Gtk::MessageDialog dialog(
        (Gtk::Window &)(*this->get_toplevel()), "This will flat the current curve, are you sure?",
...
void EqMainWindow::onBandChange(int iBand, int iField, float fValue) {
    ...
    switch (iField) {
        case GAIN_TYPE:
            write_function(controller, iBand + PORT_OFFSET + 2 * m_iNumOfChannels, sizeof(float), 0,
                           &fValue);
        ...
        case FREQ_TYPE:
            write_function(controller, iBand + PORT_OFFSET + 2 * m_iNumOfChannels + m_iNumOfBands,
        ...
        case Q_TYPE:
            write_function(controller,
                           iBand + PORT_OFFSET + 2 * m_iNumOfChannels + 2 * m_iNumOfBands,
                           sizeof(float), 0, &fValue);
        ...
        case FILTER_TYPE:
            write_function(controller,
                           iBand + PORT_OFFSET + 2 * m_iNumOfChannels + 3 * m_iNumOfBands,
                           sizeof(float), 0, &fValue);
        ...
        case ONOFF_TYPE:
            ...
            write_function(controller,
                           iBand + PORT_OFFSET + 2 * m_iNumOfChannels + 4 * m_iNumOfBands,
                           sizeof(float), 0, &fVal);
...
void EqMainWindow::onBandCtlMidSideChanged(int band) {
    ...
    write_function(controller, band + PORT_OFFSET + 2 * m_iNumOfChannels + 4 * m_iNumOfBands,
                   sizeof(float), 0, &fValue);
...
void EqMainWindow::onCurveChange(int band_ix, float Gain, float Freq, float Q) {
    ...
    // Gain
    write_function(controller, band_ix + PORT_OFFSET + 2 * m_iNumOfChannels, sizeof(float), 0,
                   &Gain);
    m_CurParams->setBandGain(band_ix, Gain);

    // Freq
    write_function(controller, band_ix + PORT_OFFSET + 2 * m_iNumOfChannels + m_iNumOfBands,
                   sizeof(float), 0, &Freq);
    m_CurParams->setBandFreq(band_ix, Freq);

    // Q
    write_function(controller, band_ix + PORT_OFFSET + 2 * m_iNumOfChannels + 2 * m_iNumOfBands,
                   sizeof(float), 0, &Q);
...
void EqMainWindow::onCurveBandEnable(int band_ix, bool IsEnabled) {
    ...
    float fVal = (float)ival;
    write_function(controller, band_ix + PORT_OFFSET + 2 * m_iNumOfChannels + 4 * m_iNumOfBands,
                   sizeof(float), 0, &fVal);
    m_CurParams->setBandEnabled(band_ix, IsEnabled);
}

void EqMainWindow::onBodeSelectBand(int band) { m_BandCtlArray[band]->glowBand(true); }
...
void EqMainWindow::onBandCtlUnselectBand() { m_Bode->unglowBands(); }
...
void EqMainWindow::saveToFile() {
    Gtk::FileChooserDialog *fileChosser =
        new Gtk::FileChooserDialog("Save curve to file", Gtk::FILE_CHOOSER_ACTION_SAVE);
...
void EqMainWindow::loadFromFile() {
    Gtk::FileChooserDialog *fileChosser =
        new Gtk::FileChooserDialog("Load curve from file", Gtk::FILE_CHOOSER_ACTION_OPEN);
    ...
    if (fileChosser->run() == Gtk::RESPONSE_ACCEPT) {
        ...
        } else  // This is the error case
        {
            ...
            Gtk::MessageDialog dialog((Gtk::Window &)(*this->get_toplevel()),
                                      "Error loading curve file, number of bands does not match or "
                                      "this is not a valid eq10q file.\n\rNothing is loaded.",
...
void EqMainWindow::onHoldFft_press() { m_Bode->setFftHold(true); }

void EqMainWindow::onHoldFft_release() { m_Bode->setFftHold(false); }
...
void EqMainWindow::sendAtomFftOn(bool fft_activated) {
    ...
    LV2_Atom *msg = (LV2_Atom *)lv2_atom_forge_object(
        &forge, &frame, 0, fft_activated ? uris.atom_fft_on : uris.atom_fft_off);
    lv2_atom_forge_pop(&forge, &frame);
    write_function(controller, AtomPortNumber, lv2_atom_total_size(msg), uris.atom_eventTransfer,
                   msg);
}

void EqMainWindow::onFftGainScale() { m_Bode->setFftGain(m_FftGain->get_value()); }

void EqMainWindow::onFftRangeScale() { m_Bode->setFftRange(m_FftRange->get_value()); }
...
void EqMainWindow::setStereoMode(bool isMidSide) {
    ...
    int PortNumber =
        PORT_OFFSET + 2 * m_iNumOfChannels + 5 * m_iNumOfBands + 2 * m_iNumOfChannels + 2;
    float fValue = isMidSide ? 1.0f : 0.0f;
    write_function(controller, PortNumber, sizeof(float), 0, &fValue);
}

void EqMainWindow::onLeftRightModeSelected() { setStereoMode(false); }

void EqMainWindow::onMidSideModeSelected() { setStereoMode(true); }
