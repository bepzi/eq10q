...
FaderWidget::FaderWidget(double dMax, double dMin, const char* bundlePath, Glib::ustring title)
    ...
    m_image_surface_ptr =
        Cairo::ImageSurface::create(format, m_image_ptr->get_width(), m_image_ptr->get_height());
    ...
    add_events(Gdk::BUTTON_PRESS_MASK | Gdk::BUTTON_RELEASE_MASK | Gdk::POINTER_MOTION_MASK |
               Gdk::SCROLL_MASK);
    signal_button_press_event().connect(sigc::mem_fun(*this, &FaderWidget::on_button_press_event),
                                        true);
    signal_button_release_event().connect(
        sigc::mem_fun(*this, &FaderWidget::on_button_release_event), true);
...
double FaderWidget::get_value() { return m_value; }
...
double FaderWidget::get_max() { return m_max; }

double FaderWidget::get_min() { return m_min; }
...
bool FaderWidget::on_expose_event(GdkEventExpose* event) {
    ...
    if (window) {
        ...
        m = ((double)(FADER_MARGIN + TITLE_OFFSET -
                      (height - FADER_MARGIN - m_image_surface_ptr->get_height()))) /
            (m_max - m_min);
        ...
        cr->move_to(round(width / 2) + 0.5,
                    FADER_MARGIN + m_image_surface_ptr->get_height() / 2 + TITLE_OFFSET);
        cr->line_to(round(width / 2) + 0.5,
                    height - FADER_MARGIN - m_image_surface_ptr->get_height() / 2 + TITLE_OFFSET);
        ...
        {
            yBarPosition = round((double)((int)(m * i + n +
                                                (double)(m_image_surface_ptr->get_height() / 2)))) +
                           0.5;  // Sum 0.5 to center cairo to the pixel
            cr->move_to(width / 2 - m_image_surface_ptr->get_width() / 3 - FADER_MARGIN + 2,
                        yBarPosition);
            cr->line_to(width / 2 - FADER_MARGIN, yBarPosition);
            cr->move_to(width / 2 + FADER_MARGIN, yBarPosition);
            cr->line_to(width / 2 + m_image_surface_ptr->get_width() / 3 + FADER_MARGIN - 2,
                        yBarPosition);
        }
        cr->stroke();
        cr->restore();
        ...
        for (double i = m_max; i >= m_min;
             i -=
             m_max / 2)  // The var step size is the half of max value because I like this way ;-)
        {
            std::stringstream ss;
            yBarPosition = (int)(m * i + n) + m_image_surface_ptr->get_height() / 2;
            ss << std::setprecision(3) << abs(i);
            // cr->move_to(width/2 - m_image_surface_ptr->get_width()/2 - 3*FADER_MARGIN,
            // yBarPosition - 4);
        ...
        cr->set_line_width(1);
        for (double i = m_max; i >= m_min;
             i -= m_max / 2)  // The var step size is the hlaf of max because I like this way
        {
            yBarPosition = round((double)((int)(m * i + n +
                                                (double)(m_image_surface_ptr->get_height() / 2)))) +
                           0.5;  // Sum 0.5 to center cairo to the pixel
            cr->move_to(width / 2 - m_image_surface_ptr->get_width() + FADER_MARGIN - 2,
                        yBarPosition);
            cr->line_to(width / 2 - FADER_MARGIN + 1, yBarPosition);
            cr->move_to(width / 2 + FADER_MARGIN - 1, yBarPosition);
            cr->line_to(width / 2 + m_image_surface_ptr->get_width() - FADER_MARGIN + 2,
                        yBarPosition);
        }
        cr->stroke();
        cr->restore();
        ...
        // Draw the fader icon
        cr->save();
        cr->set_source(m_image_surface_ptr, width / 2 - m_image_surface_ptr->get_width() / 2,
                       yFaderPosition);
        cr->rectangle(width / 2 - m_image_surface_ptr->get_width() / 2, yFaderPosition,
                      m_image_surface_ptr->get_width() + 1, m_image_surface_ptr->get_height() + 1);
        cr->clip();
        cr->paint();
        cr->restore();
    }
...
bool FaderWidget::on_button_press_event(GdkEventButton* event) {
    ...
    if (x > width / 2 - m_image_surface_ptr->get_width() / 2 &&
        x < width / 2 + m_image_surface_ptr->get_width() / 2 && y > yFaderPosition &&
        y < yFaderPosition + m_image_surface_ptr->get_height()) {
        if (!bMotionIsConnected) {
            m_motion_connection = signal_motion_notify_event().connect(
                sigc::mem_fun(*this, &FaderWidget::on_mouse_motion_event), true);
...
bool FaderWidget::on_mouse_motion_event(GdkEventMotion* event) {
    ...
    yPixels = event->y - m_image_surface_ptr->get_height() /
                             2;  // Offset fader icon to grab the center of the fader

    // Stoppers
    yPixels = yPixels < FADER_MARGIN ? FADER_MARGIN : yPixels;
    yPixels = yPixels > height - FADER_MARGIN - m_image_surface_ptr->get_height()
                  ? height - FADER_MARGIN - m_image_surface_ptr->get_height()
                  : yPixels;
...
FaderWidget::signal_FaderChanged FaderWidget::signal_changed() { return m_FaderChangedSignal; }