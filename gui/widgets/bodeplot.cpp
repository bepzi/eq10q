...
PlotEQCurve::PlotEQCurve(int iNumOfBands, int channels)
    ...
    signal_button_press_event().connect(sigc::mem_fun(*this, &PlotEQCurve::on_button_press_event),
                                        true);
    signal_button_release_event().connect(
        sigc::mem_fun(*this, &PlotEQCurve::on_button_release_event), true);
    signal_scroll_event().connect(sigc::mem_fun(*this, &PlotEQCurve::on_scrollwheel_event), true);

    // The timeout signal used to refresh the display is now connected at first run of
    // on_expose_event to run it with freq vector correctly initialized
    signal_motion_notify_event().connect(sigc::mem_fun(*this, &PlotEQCurve::on_mouse_motion_event),
                                         true);
    signal_leave_notify_event().connect(sigc::mem_fun(*this, &PlotEQCurve::on_mouse_leave_widget),
                                        true);
...
void PlotEQCurve::setCenterSpan(double center, double span) {
    ...
    // Initalize the grid
    const double f_grid[GRID_VERTICAL_LINES] = {
        20.0,   30.0,   40.0,   50.0,   60.0,   70.0,   80.0,    90.0,   100.0,  200.0,
        300.0,  400.0,  500.0,  600.0,  700.0,  800.0,  900.0,   1000.0, 2000.0, 3000.0,
        4000.0, 5000.0, 6000.0, 7000.0, 8000.0, 9000.0, 10000.0, 20000.0};
    ...
    for (int i = 0; i < CURVE_NUM_OF_POINTS; i++) {
        xPixels[i] = (double)(i) * (((double)(width - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_X)) /
                                    ((double)(CURVE_NUM_OF_POINTS - 1)));
...
void PlotEQCurve::recomputeMinFreq_fromX1Pixel(double x1) {
    ...
    double fmin = MIN_FREQ * pow((MAX_FREQ / MIN_FREQ),
                                 ((local_x1 - 3.5) / ((double)m_zoom_surface_ptr->get_width())));
    double fmax = MIN_FREQ * pow((MAX_FREQ / MIN_FREQ),
                                 ((local_x2 + 3.5) / ((double)m_zoom_surface_ptr->get_width())));
...
bool PlotEQCurve::on_button_press_event(GdkEventButton* event) {
    ...
    // Check if is a double click or simple
    if (event->button == 1 &&
        (m_zoom_widget.center_focus || m_zoom_widget.f1_focus || m_zoom_widget.f2_focus)) {
        if (event->type == GDK_2BUTTON_PRESS)  // Double click on the 1st button
        {
            // Reset freq zoom
            resetCenterSpan();
...
bool PlotEQCurve::on_mouse_motion_event(GdkEventMotion* event) {
    ...
    if (bMotionIsConnected) {
        ...
        double xclipped =
            x > width - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_X - BALL_DETECTION_PIXELS
                ? width - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_X - BALL_DETECTION_PIXELS
                : x;
        xclipped = xclipped < BALL_DETECTION_PIXELS ? BALL_DETECTION_PIXELS : xclipped;
        m_filters[m_iBandSel]->Freq = Pixels2freq(xclipped);
        m_filters[m_iBandSel]->Freq =
            m_filters[m_iBandSel]->Freq > FREQ_MAX ? FREQ_MAX : m_filters[m_iBandSel]->Freq;
        m_filters[m_iBandSel]->Freq =
            m_filters[m_iBandSel]->Freq < FREQ_MIN ? FREQ_MIN : m_filters[m_iBandSel]->Freq;

        if (m_filters[m_iBandSel]->fType == PEAK || m_filters[m_iBandSel]->fType == HIGH_SHELF ||
            m_filters[m_iBandSel]->fType == LOW_SHELF) {
            m_filters[m_iBandSel]->Gain = Pixels2dB(y);
            m_filters[m_iBandSel]->Gain =
                m_filters[m_iBandSel]->Gain > GAIN_MAX ? GAIN_MAX : m_filters[m_iBandSel]->Gain;
            m_filters[m_iBandSel]->Gain =
                m_filters[m_iBandSel]->Gain < GAIN_MIN ? GAIN_MIN : m_filters[m_iBandSel]->Gain;
        }
        ...
        m_BandEnabledSignal.emit(m_iBandSel, m_filters[m_iBandSel]->bIsOn);
        m_BandChangedSignal.emit(m_iBandSel, m_filters[m_iBandSel]->Gain,
                                 m_filters[m_iBandSel]->Freq, m_filters[m_iBandSel]->Q);
    } else {
        redraw_cursor(event->x - CURVE_MARGIN - CURVE_TEXT_OFFSET_X, event->y - CURVE_MARGIN);

        // Check if is over Zoom widget
        if ((event->x > m_zoom_widget.x1 - 10 && event->x < m_zoom_widget.x2 + 10 &&
            ...
            if (m_zoom_widget.center_press) {
                ...
                get_pointer(
                    x,
                    y);  // Using get_pinter() method is better than event->x here to avoid delays
                         // caused by unhalded mouse events due recomputeCenterFreq processing time
            ...
            else if (event->x > m_zoom_widget.x1 + 10 && event->x < m_zoom_widget.x2 - 10) {
        ...
        } else if (m_zoom_widget.center_focus || m_zoom_widget.f1_focus ||
                   m_zoom_widget.f2_focus)  // Mouse has leaved zoom widget
        {
            m_zoom_widget.center_focus = false;
            m_zoom_widget.f1_focus = false;
            m_zoom_widget.f2_focus = false;
        ...
        for (int i = 0; i < m_TotalBandsCount; i++) {
            if (x > freq2Pixels(m_filters[i]->Freq) - BALL_DETECTION_PIXELS &&
                x < freq2Pixels(m_filters[i]->Freq) + BALL_DETECTION_PIXELS &&
                y > dB2Pixels(m_filters[i]->Gain) - BALL_DETECTION_PIXELS &&
                y < dB2Pixels(m_filters[i]->Gain) + BALL_DETECTION_PIXELS && x > 0 &&
                x < width - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_X && y > 0 &&
    ...
    if (event->x > CURVE_MARGIN + CURVE_TEXT_OFFSET_X && event->x < width - CURVE_MARGIN &&
        event->y > CURVE_MARGIN && event->y < height - CURVE_MARGIN - CURVE_TEXT_OFFSET_Y) {
        // Mouse on curve zone
...
bool PlotEQCurve::on_expose_event(GdkEventExpose* event) {
    ...
    if (window) {
        ...
        if (!(m_background_surface_ptr || m_fft_surface_ptr || m_zoom_surface_ptr ||
              m_maincurve_surface_ptr || m_grid_surface_ptr || m_xAxis_surface_ptr ||
              m_yAxis_surface_ptr || m_cursor_surface_ptr)) {
            m_background_surface_ptr =
                Cairo::ImageSurface::create(Cairo::FORMAT_ARGB32, width, height);
            m_fft_surface_ptr = Cairo::ImageSurface::create(
                Cairo::FORMAT_ARGB32, width - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_X,
                height - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_Y);
            m_zoom_surface_ptr = Cairo::ImageSurface::create(
                Cairo::FORMAT_ARGB32, width - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_X,
                CURVE_TEXT_OFFSET_Y - ZOOM_WIDGET_BORDER_Y);

            // Set initial x1 and x2
            redraw_zoom_widget();

            m_maincurve_surface_ptr = Cairo::ImageSurface::create(
                Cairo::FORMAT_ARGB32, width - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_X,
                height - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_Y);
            for (int i = 0; i < m_TotalBandsCount; i++) {
                m_curve_surface_ptr[i] = Cairo::ImageSurface::create(
                    Cairo::FORMAT_ARGB32, width - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_X,
                    height - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_Y);
            }
            m_grid_surface_ptr = Cairo::ImageSurface::create(
                Cairo::FORMAT_ARGB32, width - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_X,
                height - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_Y);
            m_xAxis_surface_ptr = Cairo::ImageSurface::create(
                Cairo::FORMAT_ARGB32, width - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_X,
                CURVE_TEXT_OFFSET_Y);
            m_yAxis_surface_ptr = Cairo::ImageSurface::create(
                Cairo::FORMAT_ARGB32, CURVE_TEXT_OFFSET_X, height - CURVE_TEXT_OFFSET_Y);
            m_cursor_surface_ptr = Cairo::ImageSurface::create(
                Cairo::FORMAT_ARGB32, width - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_X,
                height - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_Y);
            redraw_background_widget();
            resetCenterSpan();
            Glib::signal_timeout().connect(
                sigc::mem_fun(*this, &PlotEQCurve::on_timeout_redraw),
                AUTO_REFRESH_TIMEOUT_MS);  // Connectin timer here to avoid using plot with
                                           // unitialized freq vector
        }
        ...
        if (m_zoom_surface_ptr) {
            cr->save();
            cr->set_source(m_zoom_surface_ptr, CURVE_MARGIN + CURVE_TEXT_OFFSET_X,
                           height - CURVE_MARGIN - CURVE_TEXT_OFFSET_Y + ZOOM_WIDGET_BORDER_Y);
            cr->paint();
            cr->restore();
        }
        ...
        if (m_xAxis_surface_ptr) {
            cr->save();
            cr->set_source(m_xAxis_surface_ptr, CURVE_MARGIN + CURVE_TEXT_OFFSET_X,
                           height - CURVE_MARGIN - CURVE_TEXT_OFFSET_Y);
            cr->paint();
            cr->restore();
        }
        ...
        if (m_maincurve_surface_ptr) {
            cr->save();
            cr->set_source(m_maincurve_surface_ptr, CURVE_MARGIN + CURVE_TEXT_OFFSET_X,
                           CURVE_MARGIN);
            cr->paint();
            cr->restore();
        }
        ...
        cr->set_line_width(1);
        cr->rectangle(CURVE_MARGIN + CURVE_TEXT_OFFSET_X + 0.5, CURVE_MARGIN + 0.5,
...
double PlotEQCurve::dB2Pixels(double db) {
    return ((((double)height) / 2.0) -
            ((((double)height) - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_Y) / m_dB_plot_range) * db -
            CURVE_TEXT_OFFSET_Y / 2 - CURVE_MARGIN);
}

double PlotEQCurve::freq2Pixels(double f) {
    return ((((double)width) - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_X) /
            (log10(m_maxFreq / m_minFreq)) *
            log10(f / m_minFreq));  // + CURVE_MARGIN + CURVE_TEXT_OFFSET_X);
}

double PlotEQCurve::Pixels2dB(double pixels) {
    return m_dB_plot_range *
           ((((double)height) - CURVE_TEXT_OFFSET_Y - 2 * CURVE_MARGIN - 2 * pixels) /
            (2 * ((double)height) - 4 * CURVE_MARGIN - 2 * CURVE_TEXT_OFFSET_Y));
}

double PlotEQCurve::Pixels2freq(double pixels) {
    return m_minFreq * pow(10, ((pixels /*- CURVE_MARGIN- CURVE_TEXT_OFFSET_X */) /
                                ((((double)width) - 2 * CURVE_MARGIN - CURVE_TEXT_OFFSET_X) /
                                 (log10(m_maxFreq / m_minFreq)))));
}

PlotEQCurve::signal_BandChanged PlotEQCurve::signal_changed() { return m_BandChangedSignal; }

PlotEQCurve::signal_BandEnabled PlotEQCurve::signal_enabled() { return m_BandEnabledSignal; }

PlotEQCurve::signal_BandSelected PlotEQCurve::signal_selected() { return m_BandSelectedSignal; }
...
void PlotEQCurve::CalcBand_DigitalFilter(int bd_ix) {
    ...
    for (int i = 0; i < CURVE_NUM_OF_POINTS; i++) {
        ...
        C = m_fil.a1 + CK * cosW;
        D = DK * sinW;
        band_y[bd_ix][i] = (double)20 * log10(sqrt(pow(A * C + B * D, 2) + pow(B * C - A * D, 2)) /
                                              (C * C + D * D));
    }
    ...
    if (m_fil.filter_order) {
        ...
        for (int i = 0; i < CURVE_NUM_OF_POINTS; i++) {
            ...
            band_y[bd_ix][i] +=
                (double)20 *
                log10(sqrt(pow(A * C + B * D, 2) + pow(B * C - A * D, 2)) / (C * C + D * D));
        }
    }
}
    ...
    if (samplerate != SampleRate) {
        ...
        if (!(m_background_surface_ptr || m_fft_surface_ptr || m_zoom_surface_ptr ||
            ...
            for (int i = 0; i <= (FFT_N / 2); i++) {
                ...
                fft_plot[i] = 0.0;
                fft_ant_data[i] = 0.0;
                // std::cout<<"freq["<<i<<"] = "<<  fft_raw_freq[i]<< "Pixels = "<< xPixels_fft[i]
                // <<std::endl;
            }
...
void PlotEQCurve::setFftHold(bool hold) { m_bFftHold = hold; }

void PlotEQCurve::setFftGain(double g) { fft_gain = g; }

void PlotEQCurve::setFftRange(double r) { fft_range = r; }
...
void PlotEQCurve::redraw_background_widget() {
    if (m_background_surface_ptr) {
        ...
        cr->arc(width - CURVE_BORDER - radius, CURVE_BORDER + radius, radius, -90 * degrees,
                0 * degrees);
        cr->arc(width - CURVE_BORDER - radius, height - CURVE_BORDER - radius, radius, 0 * degrees,
                90 * degrees);
        cr->arc(CURVE_BORDER + radius, height - CURVE_BORDER - radius, radius, 90 * degrees,
                180 * degrees);
        ...
        Cairo::RefPtr<Cairo::LinearGradient> bkg_gradient_ptr = Cairo::LinearGradient::create(
            width / 2, CURVE_BORDER, width / 2, height - CURVE_BORDER);
...
void PlotEQCurve::redraw_zoom_widget() {
    if (m_zoom_surface_ptr) {
        ...
        cr->arc(m_zoom_surface_ptr->get_width() - 3.5, m_zoom_surface_ptr->get_height() - 3.5, 3,
                0.0, 0.5 * M_PI);
        ...
        int gridPix[4];
        for (int i = 0; i < 4; i++) {
            gridPix[i] = round(((double)m_zoom_surface_ptr->get_width()) /
                               (log10(MAX_FREQ / MIN_FREQ)) * log10(f_grid[i] / MIN_FREQ)) +
                         CURVE_MARGIN + CURVE_TEXT_OFFSET_X;
        }

        // Hz scale 20 Hz
        cr->move_to(gridPix[0] - 5 - CURVE_MARGIN - CURVE_TEXT_OFFSET_X,
                    0.5 * m_zoom_surface_ptr->get_height() - 4);
        pangoLayout->set_text("20");
        pangoLayout->show_in_cairo_context(cr);
        cr->stroke();

        // Hz scale 100 Hz
        cr->move_to(gridPix[1] - 5 - CURVE_MARGIN - CURVE_TEXT_OFFSET_X,
                    0.5 * m_zoom_surface_ptr->get_height() - 4);
        ...
        // Hz scale 1 kHz
        cr->move_to(gridPix[2] - 5 - CURVE_MARGIN - CURVE_TEXT_OFFSET_X,
                    0.5 * m_zoom_surface_ptr->get_height() - 4);
        ...
        // Hz scale 10 kHz
        cr->move_to(gridPix[3] - 5 - CURVE_MARGIN - CURVE_TEXT_OFFSET_X,
                    0.5 * m_zoom_surface_ptr->get_height() - 4);
        ...
        // Get pixel position in surface acording max freq span range (freq2pixels function is not
        // valid here!)
        m_zoom_widget.x1 = round(((double)m_zoom_surface_ptr->get_width()) /
                                 (log10(MAX_FREQ / MIN_FREQ)) * log10(m_minFreq / MIN_FREQ)) +
                           3.5;
        m_zoom_widget.x2 = round(((double)m_zoom_surface_ptr->get_width()) /
                                 (log10(MAX_FREQ / MIN_FREQ)) * log10(m_maxFreq / MIN_FREQ)) -
                           3.5;
        ...
        Cairo::RefPtr<Cairo::LinearGradient> bkg_gradient_ptr =
            Cairo::LinearGradient::create(0, 0, 0, m_zoom_surface_ptr->get_height());
        ...
        pangoLayout->show_in_cairo_context(cr);
        cr->move_to(m_zoom_widget.x1 + 0.5 * (m_zoom_widget.x2 - m_zoom_widget.x1) - 15,
                    m_zoom_surface_ptr->get_height() / 2 - 4);
...
void PlotEQCurve::redraw_curve_widgets(int band) {
    if (m_curve_surface_ptr[band]) {
        ...
        Cairo::RefPtr<Cairo::LinearGradient> bd_gradient_ptr =
            Cairo::LinearGradient::create(0, Y_fil0, 0, Y_fil1);
        if (m_filters[band]->bIsOn and !m_Bypass)  // If band is enabled and not bypass
        {
            Gdk::Color color(bandColorLUT[band]);
            bd_gradient_ptr->add_color_stop_rgba(0, color.get_red_p(), color.get_green_p(),
                                                 color.get_blue_p(), 0.3);
            bd_gradient_ptr->add_color_stop_rgba(0.5, color.get_red_p(), color.get_green_p(),
                                                 color.get_blue_p(), 0.01);
            bd_gradient_ptr->add_color_stop_rgba(1, color.get_red_p(), color.get_green_p(),
                                                 color.get_blue_p(), 0.3);
    ...
    if (m_maincurve_surface_ptr) {
        ...
        if (!m_Bypass) {
            ...
            for (int i = 0; i < m_TotalBandsCount; i++)  // for each band
            {
                ...
                if (m_filters[i]->fType == PEAK || m_filters[i]->fType == LOW_SHELF ||
                ...
                ball_gradient_ptr = Cairo::RadialGradient::create(ball_x - 2, ball_y - 2, 0,
                                                                  ball_x - 2, ball_y - 2, 8);
...
void PlotEQCurve::redraw_grid_widget() {
    if (m_grid_surface_ptr) {
        ...
        for (int i = -m_dB_plot_range / 2; i <= m_dB_plot_range / 2;
             i += (int)(m_dB_plot_range / 10.0)) {
...
void PlotEQCurve::redraw_cursor(double x, double y) {
    if (m_cursor_surface_ptr) {
        ...
        // Draw text with pango to grid
        if (x > 0 && x < m_cursor_surface_ptr->get_width() && y > 0 &&
...
void PlotEQCurve::redraw_fft_widget() {
    ...
    Cairo::RefPtr<Cairo::LinearGradient> fft_gradient_ptr =
        Cairo::LinearGradient::create(0, 0, 1.0, 0);
    ...
    for (int i = 1; i <= (FFT_N / 2); i++) {
        if (m_bIsSpectrogram) {
            ...
            fft_ant_data[i] = fft_raw_data[i] > fft_ant_data[i]
                                  ? fft_raw_data[i]
                                  : fft_raw_data[i] + 0.5 * fft_ant_data[i];
            val = sqrt((float)fft_ant_data[i]);
        }
        fft_plot[i] =
            m * (20.0f * fastLog10((int*)(&val), fft_log_lut) + fft_gain + fft_pink_noise[i]);

        if (xPixels_fft_bins[i] == xPixels_fft_bins[i - 1]) {
            // Inside bin code
            binMax = fft_plot[i] < binMax
                         ? fft_plot[i]
                         : binMax;  // Yes it is reversed because binMax is really a pixel min
        } else {
            binX[binCount] = xPixels_fft_bins[i - 1];
            binY[binCount] = binMax;
            fft_gradient_ptr->add_color_stop_rgba(binX[binCount], 0.5, -1.0 * binMax + 1.0, 1.0,
                                                  -1.0 * binMax + 1.0);
            binCount++;
            binMax = fft_plot[i];
        }
    ...
    Cairo::RefPtr<Cairo::ImageSurface> img_ant = Cairo::ImageSurface::create(
        Cairo::FORMAT_ARGB32, m_fft_surface_ptr->get_width(), m_fft_surface_ptr->get_height());
    ...
    if (m_bIsSpectrogram) {
        ...
        cr->rectangle(0, SPECTROGRAM_LINE_THICKNESS, m_fft_surface_ptr->get_width(),
                      m_fft_surface_ptr->get_height() - SPECTROGRAM_LINE_THICKNESS);
        cr->fill();
        cr->restore();
