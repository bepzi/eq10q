...
BassUpMainWindow::BassUpMainWindow(const char *uri, std::string bundlePath)
    : m_pluginUri(uri), m_bundlePath(bundlePath) {
    m_Amount = Gtk::manage(
        new KnobWidget2(0.0, 6.0, "Amount", "", (m_bundlePath + KNOB_ICON_FILE).c_str()));
...
BassUpMainWindow::~BassUpMainWindow() { delete m_Amount; }
