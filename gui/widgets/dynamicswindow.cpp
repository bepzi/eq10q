...
DynMainWindow::DynMainWindow(const char *uri, std::string bundlePath, std::string title,
                             bool isCompressor, bool hasSideChain)
    : m_pluginUri(uri), m_bundlePath(bundlePath), m_bIsCompressor(isCompressor) {
    m_InGainFader =
    ...
    m_Attack = Gtk::manage(new KnobWidget2(
        0.01, 500.0, "Attack", "ms", (m_bundlePath + KNOB_ICON_FILE).c_str(), KNOB_TYPE_TIME));
    m_Release = Gtk::manage(new KnobWidget2(
        5.0, 4000.0, "Release", "ms", (m_bundlePath + KNOB_ICON_FILE).c_str(), KNOB_TYPE_TIME));
    m_Punch = Gtk::manage(new KnobWidget2(0.0, 100.0, "Punch", "%",
                                          (m_bundlePath + KNOB_ICON_FILE).c_str(), KNOB_TYPE_LIN));
    m_HPF = Gtk::manage(new KnobWidget2(20.0, 20000.0, "Key HPF", "Hz",
                                        (m_bundlePath + KNOB_ICON_FILE).c_str(), KNOB_TYPE_FREQ));
    m_LPF = Gtk::manage(new KnobWidget2(20.0, 20000.0, "Key LPF", "Hz",
                                        (m_bundlePath + KNOB_ICON_FILE).c_str(), KNOB_TYPE_FREQ));
    m_DryWet = Gtk::manage(new KnobWidget2(0.0, 100.0, "Dry/Wet", "%",
                                           (m_bundlePath + KNOB_ICON_FILE).c_str(), KNOB_TYPE_LIN));
    m_Ratio = Gtk::manage(new KnobWidget2(1.0, 100.0, "Ratio", "dB",
                                          (m_bundlePath + KNOB_ICON_FILE).c_str(), KNOB_TYPE_TIME));
    m_Knee = Gtk::manage(
        new KnobWidget2(0.0, 20.0, "Knee", "dB", (m_bundlePath + KNOB_ICON_FILE).c_str()));

    if (m_bIsCompressor) {
        // Is Compressor
        m_Hold_Makeup = Gtk::manage(
            new KnobWidget2(0.0, 20.0, "Makeup", "dB", (m_bundlePath + KNOB_ICON_FILE).c_str()));
    } else {
        // Is Gate
        m_Hold_Makeup = Gtk::manage(new KnobWidget2(
            0.01, 3000.0, "Hold", "ms", (m_bundlePath + KNOB_ICON_FILE).c_str(), KNOB_TYPE_TIME));
        m_Range = Gtk::manage(
            new KnobWidget2(-90.0, -1.0, "Range", "dB", (m_bundlePath + KNOB_ICON_FILE).c_str()));
    }
    ...
    m_LTitle.set_markup("<span font_weight=\"bold\" font=\"12px\" font_family=\"Monospace\">" +
                        title + "</span>");
    ...
    if (m_bIsCompressor) {
        m_FeedBackMode_SideChainActive.signal_clicked().connect(
            sigc::mem_fun(*this, &DynMainWindow::onFeedbackModeChange));
        m_OptoMode.signal_clicked().connect(
            sigc::mem_fun(*this, &DynMainWindow::onModeCompressorChange));
