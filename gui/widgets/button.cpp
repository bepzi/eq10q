...
Button::Button(const Glib::ustring& label) : m_label(label), m_bFocus(false), m_bPress(false) {
    ...
    signal_button_release_event().connect(sigc::mem_fun(*this, &Button::on_button_release_event),
                                          true);
    signal_motion_notify_event().connect(sigc::mem_fun(*this, &Button::on_mouse_motion_event),
                                         true);
...
bool Button::on_button_press_event(GdkEventButton* event) {
    ...
    if (event->button == 1 && event->type == GDK_BUTTON_PRESS) {
        m_bPress = event->x > OUTER_BORDER && event->x < (width - OUTER_BORDER) &&
                   event->y > OUTER_BORDER && event->y < (height - OUTER_BORDER);
        m_sigPress.emit();
        redraw();
    }
    return true;
...
bool Button::on_button_release_event(GdkEventButton* event) {
    if (event->x > OUTER_BORDER && event->x < (width - OUTER_BORDER) && event->y > OUTER_BORDER &&
...
bool Button::on_mouse_motion_event(GdkEventMotion* event) {
    m_bFocus = event->x > OUTER_BORDER && event->x < (width - OUTER_BORDER) &&
               event->y > OUTER_BORDER && event->y < (height - OUTER_BORDER);
    redraw();
    return true;
}
...
Button::signal_Click Button::signal_clicked() { return m_sigClick; }

Button::signal_Click Button::signal_press() { return m_sigPress; }

Button::signal_Click Button::signal_release() { return m_sigRelease; }
...
bool Button::on_expose_event(GdkEventExpose* event) {
    ...
    if (window) {
        ...
        cr->arc(width - OUTER_BORDER - radius, OUTER_BORDER + radius, radius, -90 * degrees,
                0 * degrees);
        cr->arc(width - OUTER_BORDER - radius, height - OUTER_BORDER - radius, radius, 0 * degrees,
                90 * degrees);
        cr->arc(OUTER_BORDER + radius, height - OUTER_BORDER - radius, radius, 90 * degrees,
                180 * degrees);
