...
typedef struct {
    ...
    // Plugin DSP
    Filter *ProcFilter[NUM_BANDS][NUM_CHANNELS];  // Dummy pointers to Filter structures, used in
                                                  // processing loop. Can point to PortFilter or
                                                  // FlatFilter depending on the MidSide option
    Filter *PortFilter[NUM_BANDS];  // Filter used for reading LV2 ports and containing the actual
                                    // coeficients
    Filter *FlatFilter;  // Allways contains coeficients for a flat filter in order to be used as a
                         // bypass in MidSide processing option
...
static void cleanupEQ(LV2_Handle instance) {
    ...
    fftw_free(plugin->fft_in);
    fftw_free(plugin->fft_out);
    fftw_destroy_plan(plugin->fft_p2);
    fftw_free(plugin->fft_in2);
    fftw_free(plugin->fft_out2);
    free(instance);
}
...
static void connectPortEQ(LV2_Handle instance, uint32_t port, void *data) {
    ...
    switch (port) {
        ...
        default:
            ...
            else if (port >= (PORT_OFFSET + 2 * NUM_CHANNELS) &&
                     port < (PORT_OFFSET + 2 * NUM_CHANNELS + NUM_BANDS)) {
                plugin->fBandGain[port - PORT_OFFSET - 2 * NUM_CHANNELS] = data;
            }

            // Connect BandFreq ports
            else if (port >= (PORT_OFFSET + 2 * NUM_CHANNELS + NUM_BANDS) &&
                     port < (PORT_OFFSET + 2 * NUM_CHANNELS + 2 * NUM_BANDS)) {
                plugin->fBandFreq[port - PORT_OFFSET - 2 * NUM_CHANNELS - NUM_BANDS] = data;
            }

            // Connect BandParam ports
            else if (port >= (PORT_OFFSET + 2 * NUM_CHANNELS + 2 * NUM_BANDS) &&
                     port < (PORT_OFFSET + 2 * NUM_CHANNELS + 3 * NUM_BANDS)) {
                plugin->fBandParam[port - PORT_OFFSET - 2 * NUM_CHANNELS - 2 * NUM_BANDS] = data;
            }

            // Connect BandType ports
            else if (port >= (PORT_OFFSET + 2 * NUM_CHANNELS + 3 * NUM_BANDS) &&
                     port < (PORT_OFFSET + 2 * NUM_CHANNELS + 4 * NUM_BANDS)) {
                plugin->fBandType[port - PORT_OFFSET - 2 * NUM_CHANNELS - 3 * NUM_BANDS] = data;
            }

            // Connect BandEnabled ports
            else if (port >= (PORT_OFFSET + 2 * NUM_CHANNELS + 4 * NUM_BANDS) &&
                     port < (PORT_OFFSET + 2 * NUM_CHANNELS + 5 * NUM_BANDS)) {
                plugin->fBandEnabled[port - PORT_OFFSET - 2 * NUM_CHANNELS - 4 * NUM_BANDS] = data;
            }

            // Connect VuInput ports
            else if (port >= (PORT_OFFSET + 2 * NUM_CHANNELS + 5 * NUM_BANDS) &&
                     port < (PORT_OFFSET + 2 * NUM_CHANNELS + 5 * NUM_BANDS + NUM_CHANNELS)) {
                plugin->fVuIn[port - PORT_OFFSET - 2 * NUM_CHANNELS - 5 * NUM_BANDS] = data;
            }

            // Connect VuOutput ports
            else if (port >= (PORT_OFFSET + 2 * NUM_CHANNELS + 5 * NUM_BANDS + NUM_CHANNELS) &&
                     port < (PORT_OFFSET + 2 * NUM_CHANNELS + 5 * NUM_BANDS + 2 * NUM_CHANNELS)) {
                plugin
                    ->fVuOut[port - PORT_OFFSET - 2 * NUM_CHANNELS - 5 * NUM_BANDS - NUM_CHANNELS] =
                    data;
            }
            ...
            // Connect Atom control_port input port from GUI
                     ...
                     PORT_OFFSET + 2 * NUM_CHANNELS + 5 * NUM_BANDS + 2 * NUM_CHANNELS + 1) {
                plugin->control_port = (const LV2_Atom_Sequence *)data;
            }

            // Connect the MidSide Mode port only for stereo versions
            else if (port ==
                     PORT_OFFSET + 2 * NUM_CHANNELS + 5 * NUM_BANDS + 2 * NUM_CHANNELS + 2) {
#if NUM_CHANNELS == 2
...
static LV2_Handle instantiateEQ(const LV2_Descriptor *descriptor, double s_rate, const char *path,
    ...
    calcCoefs(plugin_data->FlatFilter, 0.0, 20.0, 1.0, F_PEAK,
              0.0);  // Create a always-flat filter in FlatFilter

    for (i = 0; i < NUM_BANDS; i++) {
        plugin_data->PortFilter[i] = FilterInit(s_rate);
        for (ch = 0; ch < NUM_CHANNELS; ch++) {
            flushBuffers(&plugin_data->buf[i][ch]);
            plugin_data->ProcFilter[i][ch] =
                plugin_data
                    ->PortFilter[i];  // Initially all filters points to LV2 Port controlled filters
        }
#if NUM_CHANNELS == 2
    ...
    plugin_data->fft_p = fftw_plan_r2r_1d(FFT_N, plugin_data->fft_in, plugin_data->fft_out,
                                          FFTW_R2HC, FFTW_ESTIMATE);
    plugin_data->fft_p2 = fftw_plan_r2r_1d(FFT_N, plugin_data->fft_in2, plugin_data->fft_out2,
                                           FFTW_R2HC, FFTW_ESTIMATE);
    ...
    for (i = 0; i < FFT_N; i++) {
        ...
        plugin_data->fft_out2[i] =
            0;  // First fft_out2 samples will not be calculated by FFT (first-time shift)
    }
...
static void runEQ_v2(LV2_Handle instance, uint32_t sample_count) {
    ...
    lv2_atom_forge_set_buffer(&plugin_data->forge, (uint8_t *)plugin_data->notify_port,
                              notify_capacity);
        ...
        if (dB2Lin(*(plugin_data->fBandGain[bd])) != plugin_data->PortFilter[bd]->gain ||
            ...
            *plugin_data->fBandParam[bd] != plugin_data->PortFilter[bd]->q ||
            ...
            ((float)(0x01 & ((int)(*plugin_data->fBandEnabled[bd])))) !=
                plugin_data->PortFilter[bd]->enable) {
            recalcCoefs[bd] = 1;
            forceRecalcCoefs = 1;
        } else {
            recalcCoefs[bd] = 0;
...
// Check mid-side ports
    ...
    if (plugin_data->control_port) {
        ...
        while (!lv2_atom_sequence_is_end(&plugin_data->control_port->body,
                                         plugin_data->control_port->atom.size, ev)) {
            ...
            if (ev->body.type == plugin_data->uris.atom_Object) {
                ...
                if (obj->body.otype == plugin_data->uris.atom_fft_on) {
                    ...
                    lv2_atom_forge_object(&plugin_data->forge, &frameSR, 0,
                                          plugin_data->uris.atom_sample_rate_response);
    ...
    for (pos = 0; pos < sample_count; pos++) {
        ...
        if (!iBypass) {
            ...
            if (plugin_data->fft_on) {
                // Hanning Windowing
                plugin_data->fft_in[plugin_data->fft_ix] =
                    fftInSample * 0.5 *
                    (1.0 - cos((2.0 * PI * ((double)plugin_data->fft_ix)) / ((double)(FFT_N - 1))));
                plugin_data->fft_in2[plugin_data->fft_ix2] =
                    fftInSample * 0.5 *
                    (1.0 -
                     cos((2.0 * PI * ((double)plugin_data->fft_ix2)) / ((double)(FFT_N - 1))));
                ...
                if (plugin_data->fft_ix == FFT_N) {
                    ...
                    for (ffti = 0; ffti <= FFT_N / 2; ffti++) {
                        ...
                        plugin_data->fft_out[ffti] =
                            0.5 * (plugin_data->fft_normalization * (real * real + img * img) +
                                   plugin_data->fft_out2[ffti]);
                    }
                    ...
                    lv2_atom_forge_frame_time(&plugin_data->forge, 0);
                    lv2_atom_forge_object(&plugin_data->forge, &frameFft, 0,
                                          plugin_data->uris.atom_fft_data_event);
                    lv2_atom_forge_key(&plugin_data->forge, plugin_data->uris.atom_fft_data_key);
                    lv2_atom_forge_vector(&plugin_data->forge, sizeof(double),
                                          plugin_data->uris.atom_Double, ((FFT_N / 2) + 1),
                                          plugin_data->fft_out);
                ...
                if (plugin_data->fft_ix2 == FFT_N) {
                    ...
                    for (ffti = 0; ffti <= FFT_N / 2; ffti++) {
                        ...
                        plugin_data->fft_out2[ffti] =
                            plugin_data->fft_normalization * (real * real + img * img);
                    }
                ...
                for (bd = 0; bd < NUM_BANDS; bd++) {
                    if (recalcCoefs[bd]) {
                        calcCoefs(plugin_data->PortFilter[bd],
                                  ...
                                  *plugin_data->fBandFreq[bd], *plugin_data->fBandParam[bd],
...
// Band0
            ...
            // Go back to LR signals, be aware that out gains and Vumeters Are M/S or L/R depending
            // on MidSide selected mode
...
static const LV2_Descriptor eqDescriptor = {EQ_URI,   instantiateEQ, connectPortEQ, NULL,
                                            runEQ_v2, NULL,          cleanupEQ,     NULL};
