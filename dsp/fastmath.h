...
// Observa que akest metode no utilitza per res el signe, de forma que fa el valor absolut de forma
// intrinseca! t'estalvies el detector de pic! x: Input value RAW BITS format IEE745 returns: Fast
// log10 value
static inline float fastLog10(int *x_bits, float *Log10LUT) {
    // log10(N) = log10(M) + E*log10(2)
    return (Log10LUT[(*x_bits & 0x7FFFFF) >> (23 - LUT_ADDR_LENGTH)] +
            (float)((0x0FF & ((*x_bits >> 23))) - 127) * 0.30102999566398119521373889472449f);
}
...
static inline float fastLog(int *x_bits, float *LogLUT) {
    // log(N) = log(M) + E*log(2)
    return (LogLUT[(*x_bits & 0x7FFFFF) >> (23 - LUT_ADDR_LENGTH)] +
            (float)((0x0FF & ((*x_bits >> 23))) - 127) * 0.693147180559945f);
}
                      ...
                      xa * (0.000209576f +
                            xa * (8.04277e-6f +
                                  xa * (2.3149e-7f +
                                        xa * (5.33025e-9f +
                                              xa * (1.02278e-10f +
                                                    xa * (1.68217e-12f + xa * 2.42083e-14f))))))));
    } else {
        return 0.001f;
    }
}
