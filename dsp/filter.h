                             ...
                             float iEnabled)  // p2 = GAIN p3 = Q
{
    ...
    // Param Interpolation
    filter->freq =
        computeParamInterpolation(fFreq, filter->freq, filter->InterK, filter->useInterpolation);
    filter->gain =
        computeParamInterpolation(fGain, filter->gain, filter->InterK, filter->useInterpolation);
    filter->q = computeParamInterpolation(fQ, filter->q, filter->InterK, filter->useInterpolation);
    filter->enable = (double)computeParamInterpolation(iEnabled, (float)filter->enable,
                                                       filter->InterK, filter->useInterpolation);
    ...
    switch (iType) {
        ...
        case F_PEAK: {
            ...
            double G1 =
                sqrtf((w02_PI22 + (A2 * w02 * PI2) / Q2) / (w02_PI22 + (w02 * PI2) / (Q2 * A2)));
            ...
            // Digital filter
            double F = fabsf(G2 - GB2);    // + 0.00000001f; ///TODO akest petit num sumat en teoria
                                           // no hi va pero he detectat div by 0
            double G00 = fabsf(G2 - 1.0);  // + 0.00000001f;  ///TODO akest petit num sumat en
                                           // teoria no hi va pero he detectat div by 0
            double F00 = fabsf(GB2 - 1.0);  // + 0.00000001f;  ///TODO akest petit num sumat en
                                            // teoria no hi va pero he detectat div by 0
            double G01 = fabsf(G2 - G1);  // + 0.00000001f;  ///TODO akest petit num sumat en teoria
                                          // no hi va pero he detectat div by 0
            double G11 = fabsf(G2 - G12);   // + 0.00000001f;  ///TODO akest petit num sumat en
                                            // teoria no hi va pero he detectat div by 0
            double F01 = fabsf(GB2 - G1);   // + 0.00000001f;  ///TODO akest petit num sumat en
                                            // teoria no hi va pero he detectat div by 0
            double F11 = fabsf(GB2 - G12);  // + 0.00000001f;  ///TODO akest petit num sumat en
                                            // teoria no hi va pero he detectat div by 0
            double W2 = sqrtf(G11 / G00) * tanf(w0 / 2.0) * tanf(w0 / 2.0);

            // Bandwidth condition
            double Aw = (w0 / (A * filter->q)) *
                        sqrtf((GB2 - A2 * A2) / (1.0 - GB2));  // Analog filter bandwidth at GB
            double DW =
                (1.0 + sqrtf(F00 / F11) * W2) * tanf(Aw / 2.0);  // Prewarped digital bandwidth
    ...
    // Print coefs
    // printf("Coefs b0=%f b1=%f b2=%f a1=%f
    // a2=%f\r\n",filter->b0,filter->b1,filter->b2,filter->a1,filter->a2); printf("Gain = %f Freq =
    // %f Q = %f\r\n", filter->gain, filter->freq, filter->q);
}

#define DENORMAL_TO_ZERO(x) \
    if (fabs(x) < (1e-300)) x = 0.0;  // Min float is 1.1754943e-38 (Min double is 2.23×10−308)
...
static inline void computeFilter(Filter *filter, Buffers *buf, double *inputSample) {
    ...
    *inputSample = (filter->b0 * buf->buf_0 + filter->b1 * buf->buf_1 + filter->b2 * buf->buf_2) *
    ...
    if (filter->filter_order) {
        ...
        *inputSample =
            (filter->b1_0 * buf->buf_e0 + filter->b1_1 * buf->buf_e1 + filter->b1_2 * buf->buf_e2) *
                filter->enable +
            (*inputSample) * (1.0 - filter->enable);
